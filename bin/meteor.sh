...
install_meteor_npm_package_json() {
  ...
  # Define the specific path to npm-shrinkwrap.json
  shrinkwrap_file="$build_dir/.app-build/bundle/programs/server/npm-shrinkwrap.json"

  # Check if the file exists and chmod it
  if [ -f "$shrinkwrap_file" ]; then
    chmod 755 "$shrinkwrap_file"
    echo "Changed permissions of $shrinkwrap_file to 755" | output "$LOG_FILE"
  else
    echo "npm-shrinkwrap.json not found at $shrinkwrap_file" | output "$LOG_FILE"
  fi
